...
| Feature ID | Block | Description | Method | Target Metric | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **VR-CPU-01** | PC & Fetch | Sequential increment, branch target, jump target, reset lock | SV TB / SVA | 100% Branch/Jump paths | Pending (Day 6) |
| **VR-CPU-02** | RegFile | 32 registers, $x_0$ constant 0, dual-read, single-write conflict | SV TB / Formal | Formal proof of $x_0=0$ | Pending (Day 3) |
| **VR-CPU-03** | ALU | 10 operations, corner values (`0x00000000`, `0xFFFFFFFF`, signs) | SV TB / Coverage | 100% Opcode & Corner bins | **PASSED (Day 2)**: 100.0% Coverage, 0 Failures (1,933 vectors) |
| **VR-CPU-04** | Decoder/Imm | All 6 instruction formats (R, I, S, B, U, J) decoded correctly | SV TB / Coverage | 100% RV32I instruction bins | Pending (Day 4) |
| **VR-CPU-05** | Load/Store | Byte, Halfword, Word accesses, alignment verification | SV TB / SVA | All access sizes & signs | Pending (Day 5) |
| **VR-BUS-01** | APB Interconnect | Setup/Access phase timing, multi-slave decoding, slave error | UVM / SVA | Zero protocol violations | Pending (Day 8-9) |
| **VR-PER-01** | GPIO | Port direction toggling, input pin sensing, interrupt generation | UVM | Read/Write/IRQ closure | Pending (Day 10) |
| **VR-PER-02** | UART | TX shift register, RX framing, baud generator correctness | UVM | Baud accuracy, loopback | Pending (Day 11) |
| **VR-PER-03** | Timer | 32-bit counter roll-over, compare match interrupt | UVM / SVA | Interrupt latency verification | Pending (Day 12) |
| **VR-INT-01** | INTC | Arbitration between GPIO, UART, Timer; masking and ACK | UVM | Priority ordering verified | Pending (Day 13) |
| **VR-SOC-01** | SoC Top | End-to-end firmware execution from SRAM | System Test | Clean test program pass | Pending (Day 26-30) |
